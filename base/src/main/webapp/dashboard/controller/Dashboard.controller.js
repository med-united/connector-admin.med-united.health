          ...
          "sap.f.ShellBarWithFlexibleColumnLayout.controller.Dashboard",
          {
            ...
            setDashboard: function () {
              ...
              runtimeConfigModel.read("/RuntimeConfigs", {
                success: function (oData) {
                  ...
                  const invalidCertCards = {
                    "SMC_KT": 0,
                    "SMC_B": 0,
                    "HBA": 0,
                    "EGK": 0
                  };
                  const verifiedPinStatus = {
                    "SMC_B": 0,
                    "HBA_CH": 0,
                    "HBA_QES": 0,
                    "EGK": 0
                  };
                  const verifiablePinStatus = {
                    "SMC_B": 0,
                    "HBA_CH": 0,
                    "HBA_QES": 0,
                    "EGK": 0
                  };
                  const transportPinStatus = {
                    "SMC_B": 0,
                    "HBA_CH": 0,
                    "HBA_QES": 0,
                    "EGK": 0
                  };
                  const blockedPinStatus = {
                    "SMC_B": 0,
                    "HBA_CH": 0,
                    "HBA_QES": 0,
                    "EGK": 0
                  };
                  const emptyPinStatus = {
                    "SMC_B": 0,
                    "HBA_CH": 0,
                    "HBA_QES": 0,
                    "EGK": 0
                  };
                  const disabledPinStatus = {
                    "SMC_B": 0,
                    "HBA_CH": 0,
                    "HBA_QES": 0,
                    "EGK": 0
                  };
                  const errorPinStatus = {
                    "SMC_B": 0,
                    "HBA_CH": 0,
                    "HBA_QES": 0,
                    "EGK": 0
                  ...
                  const stats = {
                    "VERIFIED": verifiedPinStatus,
                    "VERIFIABLE": verifiablePinStatus,
                    "TRANSPORT_PIN": transportPinStatus,
                    "BLOCKED": blockedPinStatus,
                    "EMPTY_PIN": emptyPinStatus,
                    "DISABLED": disabledPinStatus,
                    "ERROR": errorPinStatus
                  };
                  ...
                  configs.forEach(function (config) {
                    ...
                    Promise.all([
                      ...
                      if (data[0] != null) {
                        ...
                        self.readCertData(cards, cardTypes, invalidCertCards,
                            date);
                        const pinStatus = data[3];
                        self.getStatus(pinStatus, stats);
                      } else {
                        connectorData["Inactive"] = connectorData["Inactive"]
                            + 1;
                      }
                      if (numResponses == numConfigs) {
                        const oConnectors = data[2];
                        self.excludeColumns(stats);
                        self.writeConnectorData(oConnectors, numConfigs, url,
                            terminalData, numCards, connectorData);
            ...
            readCertData: function (cards, cardTypes, invalidCertCards, date) {
              for (let k = 0; k < cards.length; k++) {
                ...
                invalidCertCards[cardType] = cards[k].certificateExpirationDate
                < date ? invalidCertCards[cardType] + 1
                    : invalidCertCards[cardType];
              }
            },
            ...
            readTerminalData: function (terminals, terminalData) {
              for (let i = 0; i < terminals.length; i++) {
                ...
                terminalData["Inactive"] = terminals[i].connected
                    ? terminalData["Inactive"] : terminalData["Inactive"] + 1
              }
            },
            ...
            getStatus: function (pinStatus, stats) {
              for (let j = 0; j < pinStatus.length; j++) {
                ...
                if (type == "HBA") {
                  type = pinStatus[j].pinType == "PIN.CH" ? "HBA_CH"
                      : "HBA_QES";
                }
                ...
                if (type != "UNKNOWN") {
                  stats[pinStatus[j].status][type] = stats[pinStatus[j].status][type]
                      + 1;
                }
              }
            },
            ...
            excludeColumns: function (stats) {
              ...
              for (let status in stats) {
                ...
                for (let typ in stats[status]) {
                  ...
                  stats[status][typ] = stats[status][typ] === 0 ? "-"
                      : stats[status][typ];
                }
                if (sum === 0) {
            ...
            writeConnectorData: function (oConnectors, numConfigs, url,
                terminalData, numCards, connectorData) {
              ...
              connectorContent.push({
                ...
                "Info": connectorData["Inactive"] > 0 ? "Offline: "
                    + connectorData["Inactive"] : this.translate("AllOnline")
              });
              connectorContent.push({
                "Name": "Kartenterminals",
                "Value": terminalData["Amount"],
                "Icon": url + "dashboard/images/CardTerminal.png",
                "State": terminalData["Inactive"] > 0 ? "Error"
                    : (terminalData["Amount"] === 0) ? "Error" : "Success",
                "Info": terminalData["Inactive"] > 0 ? "Offline: "
                    + terminalData["Inactive"] : (terminalData["Amount"] === 0)
                    ? this.translate("NotConnected") : this.translate(
                        "AllOnline")
              });
              connectorContent.push({
                "Name": "Karten",
            ...
            writeCardData: function (cardTypes, invalidCertCards, stats) {
              ...
              cardContent.push({
                ...
                "pinVerified": cardTypes["EGK"] > 0 ? stats["VERIFIED"]["EGK"]
                    : "-",
                "pinVerifiable": cardTypes["EGK"] > 0
                    ? stats["VERIFIABLE"]["EGK"] : "-",
                "pinTransport": cardTypes["EGK"] > 0
                    ? stats["TRANSPORT_PIN"]["EGK"] : "-",
                ...
                "validity": cardTypes["EGK"] > 0 ? invalidCertCards["EGK"] > 0
                    ? this.translate("Expired") + invalidCertCards["EGK"]
                    : this.translate("AllValid") : "-",
                "validityState": invalidCertCards["EGK"] > 0 ? "Error"
                    : "Success"
              });
              cardContent.push({
                "cardType": "HBA",
                "Value": cardTypes["HBA"],
                "pinType": "PIN.CH",
                "pinVerified": cardTypes["HBA"] > 0
                    ? stats["VERIFIED"]["HBA_CH"] : "-",
                "pinVerifiable": cardTypes["HBA"] > 0
                    ? stats["VERIFIABLE"]["HBA_CH"] : "-",
                "pinTransport": cardTypes["HBA"] > 0
                    ? stats["TRANSPORT_PIN"]["HBA_CH"] : "-",
                "pinBlocked": cardTypes["HBA"] > 0 ? stats["BLOCKED"]["HBA_CH"]
                    : "-",
                "pinDisabled": cardTypes["HBA"] > 0
                    ? stats["DISABLED"]["HBA_CH"] : "-",
                "pinEmpty": cardTypes["HBA"] > 0 ? stats["EMPTY_PIN"]["HBA_CH"]
                    : "-",
                "pinError": cardTypes["HBA"] > 0 ? stats["ERROR"]["HBA_CH"]
                    : "-",
                "validity": cardTypes["HBA"] > 0 ? invalidCertCards["HBA"] > 0
                    ? this.translate("Expired") + invalidCertCards["HBA"]
                    : this.translate("AllValid") : "-",
                "validityState": invalidCertCards["HBA"] > 0 ? "Error"
                    : "Success"
              });
              cardContent.push({
                "cardType": "HBA",
                ...
                "pinType": "PIN.QES",
                "pinVerified": cardTypes["HBA"] > 0
                    ? stats["VERIFIED"]["HBA_QES"] : "-",
                "pinVerifiable": cardTypes["HBA"] > 0
                    ? stats["VERIFIABLE"]["HBA_QES"] : "-",
                "pinTransport": cardTypes["HBA"] > 0
                    ? stats["TRANSPORT_PIN"]["HBA_QES"] : "-",
                "pinBlocked": cardTypes["HBA"] > 0 ? stats["BLOCKED"]["HBA_QES"]
                    : "-",
                "pinDisabled": cardTypes["HBA"] > 0
                    ? stats["DISABLED"]["HBA_QES"] : "-",
              ...
              cardContent.push({
                ...
                "pinType": "PIN.SMC",
                "pinVerified": cardTypes["SMC_B"] > 0
                    ? stats["VERIFIED"]["SMC_B"] : "-",
                "pinVerifiable": cardTypes["SMC_B"] > 0
                    ? stats["VERIFIABLE"]["SMC_B"] : "-",
                "pinTransport": cardTypes["SMC_B"] > 0
                    ? stats["TRANSPORT_PIN"]["SMC_B"] : "-",
                "pinBlocked": cardTypes["SMC_B"] > 0 ? stats["BLOCKED"]["SMC_B"]
                    : "-",
                "pinDisabled": cardTypes["SMC_B"] > 0
                    ? stats["DISABLED"]["SMC_B"] : "-",
                "pinEmpty": cardTypes["SMC_B"] > 0 ? stats["EMPTY_PIN"]["SMC_B"]
                    : "-",
                "pinError": cardTypes["SMC_B"] > 0 ? stats["ERROR"]["SMC_B"]
                    : "-",
                "validity": cardTypes["SMC_B"] > 0 ? invalidCertCards["SMC_B"]
                > 0 ? this.translate("Expired") + invalidCertCards["SMC_B"]
                    : this.translate("AllValid") : "-",
                "validityState": invalidCertCards["SMC_B"] > 0 ? "Error"
                    : "Success"
              });
              cardContent.push({
                "cardType": "SMC-KT",
                ...
                "pinEmpty": "-",
                "pinError": "-",
                "validity": cardTypes["SMC_KT"] > 0 ? invalidCertCards["SMC_KT"]
                > 0 ? this.translate("Expired") + invalidCertCards["SMC_KT"]
                    : this.translate("AllValid") : "-",
                "validityState": invalidCertCards["SMC_KT"] > 0 ? "Error"
                    : "Success",
              });
            ...
            onBeforeRendering: function () {
              const oConnectorList = this.getView().byId("ConnectorListCard");
              oConnectorList.setManifest(
                  "./dashboard/resources/ConnectorList.json");
              this.onInit();
            },
