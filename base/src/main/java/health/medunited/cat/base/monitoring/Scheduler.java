...
public class Scheduler {
  ...
  public void monitorConnectors() {
    ...
    for (RuntimeConfig runtimeConfig : runtimeConfigs) {
      ...
      try {
        ...
        addMetricCurrentlyConnectedCards(connectorResponseTime, runtimeConfig,
            eventServicePortType);

        addMetricPinStatusSMCB(VERIFIABLE, connectorResponseTime, runtimeConfig,
            eventServicePortType, cardServicePortType);
        addMetricPinStatusSMCB(VERIFIED, connectorResponseTime, runtimeConfig, eventServicePortType,
            cardServicePortType);
        addMetricPinStatusSMCB(BLOCKED, connectorResponseTime, runtimeConfig, eventServicePortType,
            cardServicePortType);
        addMetricPinStatusSMCB(DISABLED, connectorResponseTime, runtimeConfig, eventServicePortType,
            cardServicePortType);
        addMetricPinStatusSMCB(EMPTY_PIN, connectorResponseTime, runtimeConfig,
            eventServicePortType, cardServicePortType);
        addMetricPinStatusSMCB(TRANSPORT_PIN, connectorResponseTime, runtimeConfig,
            eventServicePortType, cardServicePortType);
  ...
  private Callable<Long> getTimeInSecondsUntilSMCKTCardExpires(RuntimeConfig runtimeConfig,
    ...
    return () -> {
      ...
      for (int i = 0; i < numberOfCards; i++) {
        ...
        if (Objects.equals(cardType, "SMC_KT")) {
          expirationString = getCardsResponse.getCards().getCard().get(i)
              .getCertificateExpirationDate().toString();
  ...
  private void addMetricCurrentlyConnectedCards(Timer connectorResponseTime,
      RuntimeConfig runtimeConfig, EventServicePortType eventServicePortType) {
    try {
  ...
  private Callable<Integer> getCurrentlyConnectedCards(RuntimeConfig runtimeConfig,
      EventServicePortType eventServicePortType) {
    return () -> {
      GetCards getCards = new GetCards();
      ContextType contextType = new ContextType();
  ...
  private void addMetricPinStatusSMCB(String typeOfStatus, Timer connectorResponseTime,
      RuntimeConfig runtimeConfig, EventServicePortType eventServicePortType,
      ...
      Gauge<Integer> metricPinStatusSMCB = applicationRegistry.gauge(
          Metadata.builder()
            ...
            try {
              ...
              log.info("Cards with pin status " + typeOfStatus + " : " + nrOfCardsWithStatus + " "
                  + runtimeConfig.getUrl());
  ...
  private Callable<Integer> getNrOfCardsWithStatus(String typeOfStatus, RuntimeConfig runtimeConfig,
      EventServicePortType eventServicePortType, CardServicePortType cardServicePortType) {
    return () -> {
      GetCards getCards = new GetCards();
      ContextType contextType = new ContextType();
      contextType.setMandantId(runtimeConfig.getMandantId());
  ...
  private String getPinStatus(CardServicePortType cardServicePortType, ContextType contextType,
      String cardHandle, String pinType) throws FaultMessage {
    Holder<Status> status = new Holder<>();
    Holder<PinStatusEnum> pinStatusEnum = new Holder<>();
    Holder<BigInteger> leftTries = new Holder<>();

    cardServicePortType.getPinStatus(contextType, cardHandle, pinType, status, pinStatusEnum,
        leftTries);
