...
public class RiseConnector extends AbstractConnector {
  ...
  public void restart(String connectorUrl, String managementPort,
    ...
    Response login = connectAndRetrieveResponse(POST, "/api/v1/auth/login", client, connectorUrl,
        managementPort, managementCredentials);
    log.info("Login status: " + login.getStatus());
    login.close();

    Response restart = connectAndRetrieveResponse(POST, "/api/v1/mgm/reboot", client, connectorUrl,
        managementPort, managementCredentials);
  ...
  Response connectAndRetrieveResponse(String httpMethod, String path, Client client,
      String connectorUrl, String managementPort, ManagementCredentials managementCredentials) {
    WebTarget target = client.target(connectorUrl + ":" + managementPort).path(path);

    Invocation.Builder invocationBuilder = target.request(MediaType.APPLICATION_JSON)
        .cookie(sessionCookie).header("Accept", "*/*")
        ...
        .header("sec-ch-ua-platform", "Windows").header("sec-ch-ua-mobile", "?0")
        .header("Cache-Control", "no-cache").header("Accept", "application/json, text/plain, */*")
        .header("User-Agent",
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/113.0.0.0 Safari/537.36 Edg/113.0.1774.42")
        .header("Content-Type", "application/json").header("X-Requested-With", "RISEHttpRequest")
    ...
    if (Objects.equals(httpMethod, GET)) {
      ...
      return invocationBuilder
          .post(Entity.json(new ManagementCredentialsRISE(managementCredentials.getUsername(),
  ...
  public boolean isTIOnline(String connectorUrl, String managementPort,
    ...
    Response tiStatusResponse = connectAndRetrieveResponse(GET, "/api/v1/status", client,
        connectorUrl, managementPort, managementCredentials);
