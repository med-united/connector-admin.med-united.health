sap.ui.define([], function () {
  ...
  return {
    formatDateAndTimeWithTimezone: function (inputStamp) {
      ...
      if ((inputStamp == undefined) || (inputStamp.length < 4)) {
        return "";
      }
      ...
      let convertedDate = convertTZ(
          year + "/" + month + "/" + day + " " + timeOnly, "Europe/Berlin")

      let convertedOutput = trailingZero(convertedDate.getDate()) + "."
          + trailingZero(convertedDate.getMonth() + 1) + "."
          + convertedDate.getFullYear() + " " + trailingZero(
