      ...
      return AbstractDetailController.extend(
          "sap.f.ShellBarWithFlexibleColumnLayout.controller.Detail",
          {
            ...
            _onMatched: function (oEvent) {
              AbstractDetailController.prototype._onMatched.apply(this,
                  arguments);
              const sPath = "/RuntimeConfigs('" + this._entity + "')";
              const oRuntimeConfig = this.getView().getModel().getProperty(
                  sPath);
              if (!oRuntimeConfig) {
                this.getView()
                .getModel()
            ...
            reloadModels: function (oRuntimeConfig) {
                  ...
                  (response) =>
                      ...
                      .then((res) => {
                        ...
                        for (let i = 0; i < numberOfCards; i++) {
                          if (!res.data[i].readCardCertificateResponse) {
                            continue;
                          }
                          arrayData.push({
              ...
              .then((da) => {
                ...
                for (const card of cards) {
                  ...
                  const cardTerminals = this.getView().getModel(
                      "CardTerminals").getData().cardTerminals.cardTerminal;
                ...
                .then((remoteConfig) => {
                  let productCode =
                      remoteConfig.productInformation.productIdentification
                          .productCode;
                  let connectorBrand;
                  if (productCode == "secu_kon") {
                    connectorBrand = "secunet";
                  } else if (productCode == "RKONN") {
                    connectorBrand = "rise";
                  } else if (productCode == "kocobox") {
                    connectorBrand = "kocobox";
                  }
            ...
            pwdOnRestart: async function (oEvent) {
              try {
                ...
                if (!response.ok) {
                  MessageToast.show(
                      "Fehler beim Neustart. Bitte Verbindung und Kennwort pr\u00FCfen");
                }
              } catch (error) {
                console.error(error);
              }
