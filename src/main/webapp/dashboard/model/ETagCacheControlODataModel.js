sap.ui.define(
    ["sap/ui/model/odata/v2/ODataModel", "sap/ui/core/cache/CacheManager"],
    function (ODataModel, CacheManager) {
      "use strict";
      return ODataModel.extend(
          "sap.f.ShellBarWithFlexibleColumnLayout.model.ETagCacheControlODataModel",
          {
            ...
            constructor: function () {
              ...
              this.oMetadata.pLoaded = Promise.all(
                  [pMetadataLoaded, pCacheLoaded]);
            },
            _updateETag: function (oRequest, oResponse) {
              ...
              if (!oResponse.cache && "ETag" in oResponse.headers) {
                ...
                var oCacheEntry = {
                  ...
                  "body": oResponse.body,
                  "data": contentType.match(/^application\/json.*/)
                      ? JSON.parse(oResponse.body).d : oResponse.body,
                ...
                CacheManager.set("ETagUrlCache",
                    JSON.stringify(this.oETagUrlCache));

              }
              // Add an expiration in microseconds from 1970 UTC in milliseconds
              if (!oResponse.cache && "Cache-Control" in oResponse.headers
                  && oResponse.headers["Cache-Control"].match(
                      /max-age=(\d+)$/)) {
                var oNow = new Date();
                // Update Expires to the given date
                if (sUrl in this.oETagUrlCache) {
                  this.oETagUrlCache[sUrl]["Expires"] = oNow.getTime()
                      + oNow.getTimezoneOffset() * 60000 + (RegExp.$1 * 1000);
                }
              }
              return ODataModel.prototype._updateETag.apply(this, arguments);
            },
            _createRequest: function () {
              var oRequest = ODataModel.prototype._createRequest.apply(this,
                  arguments);
              var sUrl = oRequest.requestUri.replace(this.sServiceUrl + '/',
                  '');
              if (!sUrl.startsWith("/")) {
            ...
            _submitRequest: function (oRequest, fnSuccess, fnError) {
              ...
              if ("data" in oRequest && "__batchRequests" in oRequest.data) {
                ...
                for (var i = 0; i < oRequest.data.__batchRequests.length; i++) {
                  ...
                  var oNow = new Date();
                  var iNowInMicroseconds = oNow.getTime()
                      + oNow.getTimezoneOffset() * 60000;
                  var sUrl = "/" + oBatchRequest.requestUri;
                  if (oBatchRequest.method === "GET" && sUrl
                      in this.oETagUrlCache && "Expires"
                      in this.oETagUrlCache[sUrl] && iNowInMicroseconds
                      < this.oETagUrlCache[sUrl].Expires) {
                ...
                oRequest.data.__batchRequests = oRequest.data.__batchRequests.filter(
                    function (o, i) {
                      return aResponsesFromCache.filter(function (o) {
                        return o.index == i;
                      }).length == 0;
                    });
                fnSuccessWrapper = function (oData, oResponse) {
                  for (var i in aResponsesFromCache) {
                    ...
                    oRequest.data.__batchRequests.splice(oCacheItem.index, 0,
                        oCacheItem.request);
                    oData.__batchResponses.splice(oCacheItem.index, 0,
                        oCacheItem.response);
              ...
              if (oRequest.data.__batchRequests.length == 0) {
                ...
                var retVal = ODataModel.prototype._submitRequest.apply(this,
                    [oRequest, fnSuccessWrapper, fnError]);
                return retVal;
              }
            },
            _getODataHandler: function (sUrl) {
              var oHandler = ODataModel.prototype._getODataHandler.apply(this,
                  arguments);
              if (sUrl.indexOf("$batch") > -1) {
                ...
                oHandler.read = function (oResponse, oParameter) {
                  ...
                  oResponse.body = oResponse.body.replace(/304 Not Modified/g,
                      "299 Custom not modified");
            ...
            _processSuccess: function (oRequest, oResponse, fnSuccess,
                mGetEntities, mChangeEntities, mEntityTypes, bBatch,
                aRequests) {
              ...
              if ((oResponse.statusCode === 299 || oResponse.statusCode
                  === '299') && sUrl in this.oETagUrlCache) {
              ...
              return ODataModel.prototype._processSuccess.apply(this,
                  arguments);
            }
